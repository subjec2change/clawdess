---
name: dgx-spark
description: Deploy ComfyUI + Piper TTS on a GB10 DGX Spark machine via scripts/deploy-dgx-spark.sh. Handles hardware checks, Python env, model downloads, service startup, smoke tests, and lifecycle management.
metadata: {"author": "xwings", "openclaw": {"requires": {"env": ["CLAWDESS_DEPLOY_ROOT", "CLAWDESS_MODEL_ROOT"], "optional_env": []}, "bins": ["bash {baseDir}/scripts/deploy-dgx-spark.sh"]}}
---
...
The wizard runs through 10 phases:
...
9. **Lifecycle** — generates lifecycle scripts in `bin/` and writes `deployment-manifest.json`.
10. **Finalize** — commits state and writes manifest.
...
| `llm`      | minimal + vLLM for local text generation (best-effort) | Optional/deferred |
...
```bash
./scripts/deploy-dgx-spark.sh \
  --profile minimal|media|assistant|llm|all \
  --image-model <model> \
  --tts-backend <backend> \
  [--deploy-root <path>] \
  [--model-root <path>] \
...
|------|----------|-------------|
| `--profile` | Yes (non-interactive) | Deployment profile (minimal, media, assistant, llm, all) |
