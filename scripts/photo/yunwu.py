"""yunwu.ai image edit provider (OpenAI-compatible).

Calls the yunwu.ai Images Edit API (image-to-image edit of the reference image):
...
`file://` URL, or as a direct URL (`data[0].url`).
...
def generate(api_key, prompt, image_url):
    """Edit *image_url* with *prompt* via yunwu.ai; return a file:// URL to the result."""
    try:
    ...
    print(f"YUNWU submitting edit: model={DEFAULT_MODEL}, size={DEFAULT_SIZE}")
    try:
        with urllib.request.urlopen(req) as resp:
            result = json.loads(resp.read().decode())
    except urllib.request.HTTPError as exc:
        raw = exc.read().decode() if exc.fp else ""
        print(f"YUNWU edit failed ({exc.code}): {raw[:500]}")
        return None
    except Exception as exc:  # noqa: BLE001
        print(f"YUNWU edit request error: {exc}")
        return None
